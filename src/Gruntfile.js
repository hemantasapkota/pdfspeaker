...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    copy: {
      famousjs: {
        files: [{
          expand: true,
          cwd: '<%= config.app %>/../bower_components/famous/dist/',
          dest: '<%= config.app %>/js/famous/',
          src: '*.js',
        }]
      },
      famouscss: {
    ...
    compress: {
      ...
      finalWindowsApp: {
        options: {
          archive: '<%= config.dist %>/PdfSpeaker.zip'
        },
        files: [{
          expand: true,
          cwd: '<%= config.tmp %>',
    ...
    rename: {
      app: {
        files: [{
          ...
          dest: '<%= config.dist %>/PdfSpeaker.app'
        }]
      },
      zipToApp: {
        files: [{
  ...
  grunt.registerTask('chmod', 'Add lost Permissions.', function () {
    var fs = require('fs');
    fs.chmodSync('dist/PdfSpeaker.app/Contents/Frameworks/node-webkit Helper EH.app/Contents/MacOS/node-webkit Helper EH', '555');
    fs.chmodSync('dist/PdfSpeaker.app/Contents/Frameworks/node-webkit Helper NP.app/Contents/MacOS/node-webkit Helper NP', '555');
    fs.chmodSync('dist/PdfSpeaker.app/Contents/Frameworks/node-webkit Helper.app/Contents/MacOS/node-webkit Helper', '555');
    fs.chmodSync('dist/PdfSpeaker.app/Contents/MacOS/node-webkit', '555');
    fs.chmodSync('dist/PdfSpeaker.app/Contents/MacOS/node-webkit', '555');
    fs.chmodSync('dist/PdfSpeaker.app/Contents/Resources/app.nw/tools/node', '555');
  });
  ...
  grunt.registerTask('dmg', 'Create dmg from previously created app folder in dist.', function () {
    var done = this.async();
    var createDmgCommand = 'resources/mac/package.sh "PdfSpeaker"';
